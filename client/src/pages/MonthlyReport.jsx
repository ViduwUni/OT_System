                ...
                <div className="col-span-12 p-4 rounded border border-stone-300">
                    ...
                    <div className="overflow-x-auto">
                        {loading ? (
                            ...
                            <p className="text-sm text-gray-600">No data found for selected date range.</p>
                        ) : (
                                    ...
                                    {report.map((entry, index) => {
                                        ...
                                        return (
                                            <React.Fragment key={key}>
                                                ...
                                                {expandedEmployee === entry.employee_no && entry.entries?.length > 0 && (
                                                    <tr>
                                                        <td colSpan="7" className="p-2 border bg-gray-50">
                                                            <table className="w-full text-sm">
                                                                <thead>
                                                                    <tr className="bg-gray-200">
                                                                        <th className="p-1 border">Employee No</th>
                                                                        <th className="p-1 border">Name</th>
                                                                        <th className="p-1 border">Date</th>
                                                                        <th className="p-1 border">OT Normal (hrs)</th>
                                                                        <th className="p-1 border">OT Double (hrs)</th>
                                                                        <th className="p-1 border">OT Triple (hrs)</th>
                                                                        <th className="p-1 border">Night Shift</th>
                                                                    </tr>
                                                                </thead>
                                                                <tbody>
                                                                    {entry.entries.map((day, i) => (
                                                                        <tr key={i}>
                                                                            <td className="p-1 border">{entry.employee_no}</td>
                                                                            <td className="p-1 border">{entry.employee_name}</td>
                                                                            <td className="p-1 border">
                                                                                {new Date(day.date).toLocaleDateString()}
                                                                            </td>
                                                                            <td className="p-1 border">{(day.ot_normal_hours ?? 0).toFixed(2)}</td>
                                                                            <td className="p-1 border">{(day.ot_double_hours ?? 0).toFixed(2)}</td>
                                                                            <td className="p-1 border">{(day.ot_triple_hours ?? 0).toFixed(2)}</td>
                                                                            <td className="p-1 border">{day.isNightShift ? 'Yes' : 'No'}</td>
                                                                        </tr>
                                                                    ))}
                                                                </tbody>
                                                            </table>
